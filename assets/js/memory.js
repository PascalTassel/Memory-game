...
var Alert = /*#__PURE__*/function () {
  ...
  function Alert(game) {
    ...
    this.datas = game.datas; // Set storage method
    ...
    for (var i in this.buttonsElmts) {
      ...
      this["btn".concat(this.buttonsElmts[i].action)] = button;
    } // Play button onclick
  ...
  _createClass(Alert, [{
    ...
    value: function hide() {
      ...
      for (var i in this.buttonsElmts) {
        this["btn".concat(this.buttonsElmts[i].action)].classList.add('hide');
      }
    }
    /**
    ...
    value: function win() {
      ...
      var time = this.game.counter.secondsToMinutes(this.game.score);
                    ...
                    if (ranking.length) {
                      ...
                      ranking.forEach(function (party, i) {
                        ...
                        var position = String(i + 1).padStart(2, 0),
                            ...
                            dateFr = "".concat(date[0].split('-')[2], "-").concat(date[0].split('-')[1], "-").concat(date[0].split('-')[0]),
                            time = date[1];
                        content += "\n                <tr>\n                  <td class=\"rank\">".concat(position, "</td>\n                  <td class=\"player\">\n                    <strong>").concat(party.player === null ? '-' : party.player, "</strong>\n                    <span class=\"hide-md\"><br>").concat(dateFr, " <small>").concat(time, "</small></span>\n                  </td>\n                  <td class=\"date show-md\">").concat(dateFr, " <small>").concat(time, "</small></td>\n                  <td class=\"time\"><strong>").concat(_this3.game.counter.secondsToMinutes(party.score), "</strong></td>\n                </tr>");
                      });
                      content += "</tbody>\n          </table>";
                    } else {
...
function Card(value, bgPosition) {
  ...
  this.bgPosition = bgPosition; // Card value

  this.value = value; // Card visible state

  this.isVisible = false; // Card returnable state
...
var Counter = /*#__PURE__*/function () {
  ...
  function Counter(game) {
    ...
    this.durationElmt = document.getElementById('duration'); // Game instance

    this.game = game; // Elapsed time
  ...
  _createClass(Counter, [{
    ...
    value: function start() {
      ...
      this.interval = setInterval(function () {
        ...
        _this.countUpElmt.innerHTML = new Date(diffTime).toISOString().substr(14, 5); // Progress width

        var width = (self.elapsedTime + 1) / self.game.duration * 100;
        self.game.progress.move(width); // Check counter
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": function() { return /* binding */ Datas; }
/* harmony export */ });
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var Datas = /*#__PURE__*/function () {
   ...
   */
  function Datas(backupMethod, rankingLimit) {
    _classCallCheck(this, Datas);

    // Datas file
    this.endPoint = 'data'; // Backup Method

    this.backupMethod = backupMethod; // Ranking Limit
  ...
  _createClass(Datas, [{
    ...
    value: function setScore(datas) {
      ...
      datas.date = "".concat(dateString[0], "-").concat(dateString[1], "-").concat(dateString[2], " ").concat(currentDate.toLocaleTimeString(locale));

      if (this.backupMethod === 'database') {
        return fetch("".concat(this.endPoint, "/setScore"), {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
          },
    ...
    key: "formUrlEncoder",
    value: function formUrlEncoder(params) {
      var formUrlBody = [];

      for (var property in params) {
        var key = encodeURIComponent(property);
        var value = encodeURIComponent(params[property]);
        formUrlBody.push("".concat(key, "=").concat(value));
      }
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _memory_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./memory.js */ "./assets/src/js/memory.js");
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": function() { return /* binding */ Memory; }
/* harmony export */ });
/* harmony import */ var _alert_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./alert.js */ "./assets/src/js/alert.js");
/* harmony import */ var _card_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./card.js */ "./assets/src/js/card.js");
/* harmony import */ var _counter_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./counter.js */ "./assets/src/js/counter.js");
/* harmony import */ var _datas_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./datas.js */ "./assets/src/js/datas.js");
/* harmony import */ var _progress_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./progress.js */ "./assets/src/js/progress.js");
...
var Memory = /*#__PURE__*/function () {
  ...
  function Memory() {
    ...
    fetch('settings.json').then(function (response) {
      ...
      _this.countDownValueElmt = document.getElementById('countdown-value'); // Progress bar instance

      _this.progress = new _progress_js__WEBPACK_IMPORTED_MODULE_4__["default"](); // Counter instance

      _this.counter = new _counter_js__WEBPACK_IMPORTED_MODULE_2__["default"](_this); // Datas instance

      _this.datas = new _datas_js__WEBPACK_IMPORTED_MODULE_3__["default"](_this.backupMethod, _this.rankingLimit); // Alert instance

      _this.alert = new _alert_js__WEBPACK_IMPORTED_MODULE_0__["default"](_this); // Init game
  ...
  _createClass(Memory, [{
    ...
    value: function init() {
      // Ended game
      this.ended = false; // Cards game

      this.cards = []; // Current selected cards

      this.selectedCards = []; // Flipped cards

      this.flippedCards = []; // Game status

      this.isWin = false; // Player score

      this.score = 0; // Cards values

      var cardsValues = this.cardsValues; // Set cards values
      ...
      try {
        var _loop = function _loop() {
          ...
          var liElmt = document.createElement('li'),
              ...
              backElmt = document.createElement('div'); // Card elmt properties
          ...
          backElmt.innerHTML = _this2.debug ? "<span class=\"card__value\">".concat(card.value, "</span>") : ''; // Append back and front to card elmt

          liElmt.append(backElmt, frontElmt); // Store liElemt in card object

          card.elmt = liElmt; // On card elmt click

          card.elmt.addEventListener('click', function () {
            // Ended game ?
            if (_this2.ended) {
              return false;
            }

            if (_this2.selectedCards.length !== _this2.nbOccurences) {
              // Flip card
              card.elmt.classList.add('card--flipped'); // Flipped card

              card.isVisible = true; // Add to selected cards

              _this2.selectedCards.push(card.value); // Compare returned cards


              if (_this2.selectedCards.length === _this2.nbOccurences) {
                _this2.checkFlippedCards();
              }
            }
          }); // Append card to list element

          _this2.boardElmt.appendChild(card.elmt);
        };
    ...
    value: function checkFlippedCards() {
      ...
      var identical = this.selectedCards[0] === this.selectedCards[1];

      var _loop2 = function _loop2(i) {
        var card = _this3.cards[i]; // If not returned and returnable, disable card

        card.elmt.classList.toggle('card--disabled', card.isReturnable && card.isVisible === false); // If identical, blink card

        card.elmt.classList.toggle('card--blink', identical && _this3.selectedCards.includes(card.value)); // If identical, add card to flipped cards

        if (identical && _this3.selectedCards.includes(card.value)) {
          _this3.flippedCards.push(card);
        ...
        card.isVisible = false; // Hide and enable card
    ...
    value: function gameOver() {
      // Stop counter
      this.counter.stop(); // Ended game

      this.ended = true; // If player loose, display loose alert
    ...
    value: function replay() {
      ...
      this.progress.reset(); // Cards game

      this.init(); // Launch game
    ...
    value: function checkSettings(settings) {
      ...
      for (var setting in settings) {
        // Get setting value
        var settingValue = settings[setting]; // Value integrity
...
/******/ 	__webpack_require__.O(undefined, ["assets/css/memory"], function() { return __webpack_require__("./assets/src/js/index.js"); })
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["assets/css/memory"], function() { return __webpack_require__("./assets/src/scss/memory.scss"); })
